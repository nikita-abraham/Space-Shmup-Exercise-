...
public class Enemy_4 : Enemy {
	...
	void Start () {
		...
		foreach (Part prt in parts) {
			...
			if (t != null) {
				prt.go = t.gameObject;
				prt.mat = prt.go.GetComponent<Renderer>().material; // Changed for Unity 5
			}
		}
	}
	...
	public override void Move() {
		...
		if (u >= 1) {
			InitMovement();	// Initialize movement to a new point
			u=0;
		}
		
		u = 1 - Mathf.Pow (1 - u, 2);	// Apply ease out easing to u
		
		pos = (1 - u) * points[0] + u * points[1]; // Simle linear interpolation
	}
	
	// This will override the OnCollisionEnter that is part of Enemy.cs
	// Because of the way that MonoBehaviour declares commin Unity Functions
